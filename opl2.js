...
Envelope.prototype.setADSR = function(att, dec, sus, rel) {
  ...
  // decay and release use linear rates --
  // rate 4 is 8 volume levels every 512 samples
  //           1 volume level every 64 samples
  // rate 0 is 1 volume level every 64<<4 samples = 1024 samples
  this.decayInc = (1 << dec) / 1024.0;
  this.releaseInc = (1 << rel) / 1024.0;

  // verified on real chip; sustain level 1 = 128, 2 = 256, etc
  this.sustainLevel = sus << 7;
...
D00Sequencer.prototype.updateSPFX = function(song, channel) {
  if (this.spfxptr == null) {
    return;
  }
  var spfx = song.spfx[this.spfxptr];
  if (spfx == undefined) {
    throw "invalid spfx ptr " + this.spfxptr + " / " + song.spfx.length;
  }
  var mleveladd = (spfx[4] << 24) >> 24;  // sign extend
  if (this.spfxdelay > 0) {
    this.spfxdelay--;
    channel.mlevel += mleveladd << 5;
    return;
  }
  //       0     1        2      3         4        5   6  7
  // spfx: inst 00 halfnote modlev modlevadd duration ptr 00
  channel.setD00Instrument(song.instruments[spfx[0]]);
  this.instNum = spfx[0];
  channel.mlevel = spfx[3] << 5;
  channel.mlevel += mleveladd << 5;
  this.spfxdelay = 1 + spfx[5];
  this.spfxptr = spfx[6];
}
